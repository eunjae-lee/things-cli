...
/**
 * Validates a when date string - supports special values and YYYY-MM-DD format
 * @param {string} whenStr - When string to validate
 * @throws {Error} If when format is invalid
 * @returns {Object} Object with type and value
 */
export function validateWhenString(whenStr) {
  if (!whenStr) {
    throw new Error('When date cannot be empty');
  }

  // Check for special values
  const specialValues = ['today', 'tomorrow', 'anytime', 'someday'];
  if (specialValues.includes(whenStr.toLowerCase())) {
    return { type: 'special', value: whenStr.toLowerCase() };
  }

  // Check for YYYY-MM-DD format
  const dateRegex = /^\d{4}-\d{2}-\d{2}$/;
  if (dateRegex.test(whenStr)) {
    const date = new Date(whenStr);
    if (isNaN(date.getTime())) {
      throw new Error('Invalid when date provided');
    }
    return { type: 'date', value: date };
  }

  throw new Error('When date must be "today", "tomorrow", "anytime", "someday", or YYYY-MM-DD format');
}
...
export function convertTodoDateFields(todo) {
  ...
  // Add whenDate as an alias for activationDate for better API consistency
  cleaned.whenDate = cleaned.activationDate;
  
  return cleaned;
}

/**
 * Formats a date for Things URL scheme
 * @param {Date} date - JavaScript Date object
 * @returns {string} Date formatted as YYYY-MM-DD for URL scheme
 */
export function formatDateForUrlScheme(date) {
  const year = date.getFullYear();
